...
const db = new sqlite3.Database("chat.db"); // Persistent file-based database
...
io.on("connection", (socket) => {
  ...
  socket.on("chat message", ({ room, username, message, type, content }) => {
    ...
    db.run("INSERT INTO messages (room, username, message, type, content) VALUES (?, ?, ?, ?, ?)", [room, username, message, type, content], (err) => {
      if (err) {
        console.error(err.message);
        return;
      }
      // Broadcast message to the room
      io.to(room).emit("chat message", { username, message, type, content });
    });
  });
  ...
  socket.on("delete message", ({ messageId }) => {
    ...
    db.get("SELECT username FROM messages WHERE rowid = ?", [messageId], (err, row) => {
      if (err) {
        console.error(err.message);
        return;
      }

      // Check if the user requesting deletion is the message creator
